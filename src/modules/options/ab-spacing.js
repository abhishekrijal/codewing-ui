...
import InputWithOnlyNumbers from "../helpers/InputWithOnlyNumbers";
...
const Spacing = ({ value, option, onChange }) => {
	...
	const getNumericValue = (value, unit = "") => {
		...
		return `${parseFloat(value) === 0 ? 0 : parseFloat(value) || ""
			}${unit}`;
	};
			...
			{["top", "right", "bottom", "left"].map((side) => (
				<span key={side}>
					...
					<small>
						{
							{
								top: __("Top", "magblocks"),
								bottom: __("Bottom", "magblocks"),
								left: __("Left", "magblocks"),
								right: __("Right", "magblocks"),
							}[side]
						}
					</small>
				</span>
