...
export const getFirstLevelOptions = (options, hasInnerOptions = true) => {
	...
	return Object.keys(rest).reduce((currentOptions, currentOptionId) => {
		if (options[currentOptionId] && !options[currentOptionId].type) {
			return {
				...currentOptions,
		...
		if (options[currentOptionId] && options[currentOptionId].options) {
			return {
				...currentOptions,
				...getFirstLevelOptions(
		...
		if (options[currentOptionId] && options[currentOptionId]['inner-options'] && hasInnerOptions) {
			return {
				...currentOptions,
		...
		...Object.keys(firstLevelOptions).reduce(
			(currentValues, currentOptionId) => {
				...
				if (Object.keys(values).indexOf(currentOptionId) > -1) {
					...
					if (
						firstLevelOptions[currentOptionId] && Object.keys(firstLevelOptions[currentOptionId]).indexOf(
							'value'
						) > -1
					) {
