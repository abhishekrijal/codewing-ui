...
const usePopoverMaker = ({
	...
	useEffect(() => {
		...
		if (ref.current) {
			...
			if (ref.current.closest('.rt-tabs-scroll')) {
				observer.observe(ref.current.closest('.rt-tabs-scroll'), {
					attributes: true,
				})
			}

			if (ref.current.closest('.rt-modal-scroll')) {
				observer.observe(ref.current.closest('.rt-modal-scroll'), {
					attributes: true,
				})
			}
	...
	let { right, yOffset, position, otherStyles } = useMemo(() => {
		...
		if (ref.current) {
			...
			let el = ref.current.closest('.rt-select-input')
				? ref.current.closest('.rt-select-input')
				: ref.current
